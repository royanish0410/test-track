...
export default function EligibleToAttempt() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "endDate" | "teacher" | "eligibility">("eligibility")
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc")
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid")
  const [darkMode, setDarkMode] = useState(false)
  const [eligibilityFilter, setEligibilityFilter] = useState<"all" | "eligible" | "attempted">("all")
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        ...
        // Initialize eligibility status for each quiz
        const quizzesWithEligibility = result.data.map((quiz) => ({
          ...quiz,
          eligibilityStatus: {
            isEligible: false,
        ...
        setQuizzes(quizzesWithEligibility)
        setFilteredQuizzes(quizzesWithEligibility)

        // Check eligibility for each quiz
        quizzesWithEligibility.forEach((quiz) => {
          checkEligibility(quiz.id)
        })
      } catch (err) {
  ...
  const checkEligibility = async (quizId: string) => {
    try {
      const response = await fetch(`/api/quiz/student/eligible-to-attempt/${quizId}`)
      ...
      // Update the quiz with eligibility status
        ...
        prevQuizzes.map((quiz) => {
          if (quiz.id === quizId) {
            return {
              ...quiz,
              eligibilityStatus: {
                isEligible: !result.attemptStatus,
                message: result.attemptStatus
      ...
      // Update the quiz with error status
      setQuizzes((prevQuizzes) =>
        prevQuizzes.map((quiz) => {
          if (quiz.id === quizId) {
            return {
  ...
  useEffect(() => {
    const filtered = quizzes.filter((quiz) => {
      ...
      const matchesEligibility =
        ...
        (eligibilityFilter === "eligible" && quiz.eligibilityStatus?.isEligible) ||
        (eligibilityFilter === "attempted" && !quiz.eligibilityStatus?.isEligible)
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "eligibility":
          ...
          comparison =
            (a.eligibilityStatus?.isEligible === true ? 0 : 1) - (b.eligibilityStatus?.isEligible === true ? 0 : 1)
          break
      }
  ...
  }, [quizzes, searchTerm, selectedSubject, sortBy, sortOrder, eligibilityFilter])
      ...
      >
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className={`text-lg ${darkMode ? "text-gray-300" : "text-gray-600"}`}>Checking eligibility...</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="container mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          ...
          <p className={`text-lg ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
            Check which quizzes you're eligible to attempt
          </p>
        </motion.div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center">
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={(e) => setEligibilityFilter(e.target.value as "all" | "eligible" | "attempted")}
                className={`pl-10 pr-8 py-3 rounded-xl border transition-colors appearance-none ${darkMode ? "bg-slate-700/50 border-slate-600 text-white" : "bg-white border-gray-300 text-gray-900"}`}
              >
            ...
            <div className="flex items-center gap-2">
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as "name" | "endDate" | "teacher" | "eligibility")}
                ...
                >
                  ...
                  <div className="mb-4">
                    {eligibilityStatus?.loading ? (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-gray-50 dark:bg-slate-700/30">
                      ...
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-red-50 dark:bg-red-900/20">
                        ...
                        <p className="text-sm text-red-700 dark:text-red-300">Error checking eligibility</p>
                      </div>
                    ) : (
                      <div
